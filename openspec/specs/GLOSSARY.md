...
| **db** | The command group that reads the Local cache offline (e.g. `db today`, `db week`, `db trends`); its former `sync`-alias subcommand was removed in 0.5.0 — use `sync` directly (`src/commands/db.ts`, `src/db/queries.ts`). |
...
| **Output mode** | How a command formats results: **table/text** on a TTY, **JSON** when stdout is not a TTY, overridable with `--format <json\|table>` (`src/commands/common.ts`, resolved in `src/lib/format-resolve.ts`, rendered in `src/render/format.ts`). |
...
| **fetch** | The command that issues a direct, lightly-typed call to an Oura Cloud API endpoint for one collection: `oura-cli fetch <collection>` (`src/commands/fetch.ts`). |
