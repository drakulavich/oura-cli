...
export interface Collection<Row> {
   ...
   * `start_datetime`/`end_datetime` (ISO 8601 — the heartrate and battery timeseries), or `none`
   * for a snapshot endpoint such as ring_configuration that takes no range at all and is fetched
   * whole every time (`fetch` rejects range flags for it; `sync` keeps no watermark).
   */
  rangeParams: 'date' | 'datetime' | 'none';
   ...
   * only when start_date <= D < end_date ([0, 1]). Verified live against the API on 2026-09-05/06.
   */
