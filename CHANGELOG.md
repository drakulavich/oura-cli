...
- The same for the five tables whose `day` is not unique (`sleep_model`, `workouts`, `sessions`, `rest_mode_periods`, `enhanced_tags`): a record re-issued under a new id used to leave both rows, so `report` averaged one night twice. The scope of a removal always comes from the response, never from the request, and a response that would drop most of a large window is refused as truncated and reported instead. (#71)
- `sync` reports `removed` per table in its JSON, and the progress line says how many stale rows went.
