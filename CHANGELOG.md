...
- `doctor` names the rebuild when the cache file is too damaged to open at all (a truncated file, say): the `database` row now carries the same fix the DB_ERROR hint carries, so `Next:` no longer says "see the failing checks above" while `db today` on the same file gives the recipe. (#144)
...
- `sync` no longer re-fetches two weeks of heart rate on every run. The fortnight behind the watermark is re-read on the first sync of the day and whenever the run brings samples newer than the cache, which is when the ring has uploaded and Oura can have appended workout samples to older days; a repeat sync on a quiet ring skips it. Measured on a live account: 37 requests and 9.2 s became 17 requests and 3.5 s. A new `_sync_state` table (schema version 5) remembers the day the re-read last ran. (#135)
