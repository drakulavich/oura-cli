...
- `sync` no longer aborts on a heart-rate sample without a `timestamp`. The `day` column is derived from it, so a null threw a `TypeError` at insert time, outside the CLI's error mapping, and every collection after `hr` in the run never synced. A row missing any identity field is now dropped before insert, counted in the JSON under `dropped` and named on the collection's line (`1 dropped (no timestamp)`); the row was unusable anyway, since the identity is what it would have been stored under. (#106)
