...
- `--no-color` left citty's help output coloured, because citty reads `NO_COLOR` when its module is evaluated and the variable was only set afterwards. The colour decision moved into `src/lib/color-mode.ts`, imported first by the entry point. Piped help is plain too unless `FORCE_COLOR` asks otherwise. (#80)
