...
| 2026-08-08 | `src/commands/sync.ts` | `src/commands/sync.test.ts` | Characterization tests for `runSync` (sync orchestration). Boundaries mocked exactly where the code touches the outside world: HTTP via a `globalThis.fetch` stub keyed by endpoint, sqlite via a temp db file (`opts.db`); real collaborators (`importDaily`, `getDaySummary`, `formatDaySummary`, `resolveFormat`) run unmocked. `tz: 'UTC'` makes the summarized day deterministic (`= new Date().toISOString().slice(0,10)`). Covers: default format = JSON when stdout is not a TTY; unknown `--format` → `BAD_ARGS` CliError raised by `resolveFormat` **before** any fetch/db work (0 fetch calls); JSON mode prints a single `{ import, today }` object; import payload window (`endDate = today`) and per-endpoint counts (incl. zero-count endpoints still requested); today summary reads back freshly-imported rows (scores/steps/stress/spo2/temp + sleep-hour derivations 8h/2h/1.5h); JSON mode passes **no** logger to the importer (no `Syncing from`/`Import complete.` leakage); `getDaySummary` keys strictly on today so a yesterday-dated row leaves today's field null; empty API → all-null summary + zero counts; table mode forwards `console.log` to the importer (progress lines printed) and prints exactly `formatDaySummary(today, 'table')` as its final line; rows persisted to the configured db path; schema created/migrated on a brand-new db file; `TOKEN_MISSING` CliError propagates when no token resolves. No suspected bugs — `sync.ts` behaved as its code reads. |

## Queue (candidate modules, roughly prioritised)

Modules with real logic and no dedicated test file yet:

- `src/commands/db.ts` — db subcommand handlers. **Next up.**
