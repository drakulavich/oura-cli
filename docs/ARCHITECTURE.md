...
2. Create `src/collections/<name>.ts` with `defineCollection<Row>({ … })` and add it to `COLLECTIONS`. Set `rangeParams` to `'date'` unless the endpoint is a timeseries taking `start_datetime`/`end_datetime` (`'datetime'`) or takes no range at all (`'none'`, e.g. `ring_configuration`: fetched whole every sync, `fetch` rejects range flags), `maxRangeDays` if the endpoint caps the range per request (heartrate: 30), and `dayRangeOffset` if the endpoint's date bounds are not both inclusive (`sleep`: start exclusive → `[-1, 0]`; `workout`: end exclusive → `[0, 1]`) — probe a single day live before trusting the spec (check the OpenAPI spec at `https://cloud.ouraring.com/v2/static/json/openapi-1.37.json`). Set `syncLookbackDays` as well if Oura publishes rows for a day after that day has passed, the way it appends workout samples to `heartrate` (`hr`: 14); an incremental sync then re-walks that many days behind the watermark.
